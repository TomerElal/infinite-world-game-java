...
/**
 * A class that represents the game manager and starts the game.
 */
public class PepseGameManager extends GameManager {
     ...
     * Since the world is infinite, we save every moment 3 frames of the screen, that each one continues the previous.
     * The avatar is standing on the middle one, and for every move to the next frame, we change the one from the other side
     * to continue the sequence.
     * @param imageReader Contains a single method: readImage, which reads an image from disk.
     *                   See its documentation for help.
     * @param soundReader Contains a single method: readSound, which reads a wav file from disk.
     *                   See its documentation for help.
     * @param inputListener Contains a single method: isKeyPressed, which returns whether a given key is currently pressed by
     *                      the user or not. See its documentation.
    ...
    public void initializeGame(ImageReader imageReader, SoundReader soundReader,
        ...
        double avatarYCoord = MathOrLogicOperations.normalizeHeight(terrains[THE_MIDDLE_FRAME].groundHeightAt(avatarXCoord)) - MAKE_Y_LEFT_CORNER;
        avatar = Avatar.create(gameObjects(), AVATAR_LAYER, Vector2.of(avatarXCoord, (float) avatarYCoord), inputListener, imageReader);
        handleAllCollisionsBetweenObjects();
        setCamera(new Camera(avatar, Vector2.ZERO, windowDimensions, windowDimensions));
    }

    /**
     * An override to the update method that handles the recreation and deletion of the world when the avatar moves.
     * When the avatar leaves the middle frame, we delete the farther frame and create a new frame in the avatar move direction,
     * so that the avatar will always be in the middle frame.
    ...
    private void handleAllCollisionsBetweenObjects() {
        for (int i = 0; i < NUMBER_OF_FRAMES; i++) {
            ...
            gameObjects().layers().shouldLayersCollide(fallingLeafLayer[i], topGroundLayer[i], true);

            gameObjects().layers().shouldLayersCollide(fallingLeafLayer[(i + 1) % 3], topGroundLayer[i], true);

            gameObjects().layers().shouldLayersCollide(fallingLeafLayer[(i + 2) % 3], topGroundLayer[i], true);
        }
    }
